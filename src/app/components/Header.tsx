      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="md:hidden absolute top-[64px] left-0 w-full bg-white/95 backdrop-blur-lg border-b border-gray-200 shadow-md"
          >
            <nav className="flex flex-col items-start px-6 py-5 space-y-4">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
